...
const Product = () => {
    ...
    const handleAddToCart = (itemId) => {
        ...
        axiosInstance.get(`/api/cart/${itemId}`)
            .then(res => res.data) // 4. (修改) .json() 改为 .data
            .then((data) => {
                if (data.status === 0) {
    ...
    useEffect(() => {
        if (productId) {
            axiosInstance.get(`/api/catalog/products/${productId}`)
                .then(res => res.data) // 6. (修改) .json() 改为 .data
                .then(data => {
                    if (data.status === 0) {
