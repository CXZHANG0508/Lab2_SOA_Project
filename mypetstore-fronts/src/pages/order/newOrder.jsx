...
export default function NewOrder() {
    ...
    const handleConfirm = async () => {
        try {
            const res = await axiosInstance.post('/api/order', formData);
            const result = res.data;
            if (result.status === 0 && result.data?.orderId) {
                const orderId = result.data.orderId;
                alert('订单提交成功！');
                await axiosInstance.get(`/api/order/${orderId}`); // 无需 headers
                await Promise.all(
                    cartItems.map(item =>
                        axiosInstance.delete(`/api/cart/${item.cart.itemId}`) // 无需 headers
                    )
                );
